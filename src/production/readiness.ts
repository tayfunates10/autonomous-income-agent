...
export function evaluateProductionReadiness(config: ProductionConfig, now = new Date()): ReadinessReport {
  const checks: ReadinessCheck[] = [
    ...
    check("owner_public_key", config.ownerKeyId.trim().length > 0 && /BEGIN PUBLIC KEY/.test(config.ownerPublicKeyPem), "Owner approval verification public key must be configured; private signing key must remain external."),
